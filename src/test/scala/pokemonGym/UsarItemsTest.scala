...
//Toma Ether tests
  ...
  "Pikachu" should "usar piedra electrica y evolucionar" in {
    ...
    val estadoDespuesDeActividad = gim.ejecutar(pikachu, OK(pikachu), UsarPiedraParaEvolucionar(Electrico))
  ...
  "Pikachu" should "usar piedra Agua y no hacer nada" in {
    val pikachu = Pokemon(90, 150, 30, 35, 25, 'M', Especie.Pikachu(), List((Impactrueno, 4)), 55)
    val estadoDespuesDeActividad = gim.ejecutar(pikachu, OK(pikachu), UsarPiedraParaEvolucionar(Agua))
  ...
  "Pikachu" should "usar piedra Tierra y no Evolucionar y quedar Envenenado" in {
    ...
    val estadoDespuesDeActividad = gim.ejecutar(pikachu, OK(pikachu), UsarPiedraParaEvolucionar(Tierra))
  ...
  "Jigglypuff" should "usar piedra Lunar y evoluciona" in {
    ...
    val estadoDespuesDeActividad = gim.ejecutar(jigglypuff, OK(jigglypuff), UsarPiedraParaEvolucionar(Lunar))
  ...
  "Wigglytuff" should "usar piedra Lunar y no evoluciona" in {
    val wigglytuff = Pokemon(90, 150, 30, 35, 25, 'M', Especie.Wigglytuff(), List((Reposar, 4)), 55)
    val estadoDespuesDeActividad = gim.ejecutar(wigglytuff, OK(wigglytuff), UsarPiedraParaEvolucionar(Lunar))
    estadoDespuesDeActividad should be (OK(wigglytuff))
  }
